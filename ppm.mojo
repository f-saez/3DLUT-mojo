...
from colorspace import srgb_to_rec709, rec709_to_srgb
...
struct Image:
    ...
    fn convert_to_rec709(inout self):
        if self.is_srgb():
            var ps255 = SIMD[DType.float32,4](255)
            var div_ps255 = SIMD[DType.float32,4](1/255)
            var idx = 0
            for _ in  range(self.get_num_pixels()):
                var rgba = self.pixels.load[width=4](idx).cast[DType.float32]()
                rgba *= div_ps255
                srgb_to_rec709(rgba)
                rgba *= ps255
                self.pixels.store[width=4](idx, rgba.cast[DType.uint8]())
                idx += 4

    fn convert_to_srgb(inout self):
        if self.is_srgb()==False:
            var ps255 = SIMD[DType.float32,4](255)
            var div_ps255 = SIMD[DType.float32,4](1/255)    
            var idx = 0
            for _ in  range(self.get_num_pixels()):
